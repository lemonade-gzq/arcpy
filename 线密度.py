...
# env.workspace = r"E:\\城市与区域生态\\大熊猫和竹\\平武种群动态模拟\\only_ndr\\考虑季节性迁徙的模拟结果\\轨迹"  # 设置工作空间，用于后面遍历矢量文件
...
env.workspace = "E:\\城市与区域生态\\大熊猫和竹\\平武种群动态模拟\\only_ndr\\考虑季节性迁徙的模拟结果\\轨迹密度"
rasters = arcpy.ListRasters()  # 遍历工作空间路径内的所有栅格
Sum1 = 0
for raster in rasters:
    Sum1 = Sum1 + Raster(raster)
Sum1 /= len(rasters)
Sum1.save(env.workspace + '\\trajectory_average_4k.tif')
